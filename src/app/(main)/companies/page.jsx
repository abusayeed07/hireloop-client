...
const CompanyCard = ({ company, index }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm border border-zinc-200/50 dark:border-zinc-800/50 rounded-2xl p-6 transition-all duration-300 hover:border-blue-500/30 hover:shadow-lg hover:shadow-blue-500/5 flex flex-col justify-between"
    >
      <div className="flex items-start gap-4">
        {/* Company Logo */}
        <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-blue-500/10 to-purple-500/10 border border-zinc-200/50 dark:border-zinc-800/50 flex items-center justify-center overflow-hidden flex-shrink-0">
          {company.logo ? (
            <Image 
              src={company.logo} 
              alt={company.name} 
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 flex-wrap sm:flex-nowrap">
            <h3 className="text-lg font-semibold text-zinc-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition-colors cursor-pointer truncate">
              {company.name}
            </h3>
      ...
      <div className="flex items-center justify-between mt-6 pt-4 border-t border-zinc-200/50 dark:border-zinc-800/50">
...
export default function CompaniesPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [companiesResponse, statsResponse] = await Promise.all([
          ...
          serverFetch('/api/companies/stats/public')
        ]);
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-white pt-20">
      {/* Hero Section */}
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-8 py-8">
        ...
        <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-3 mb-8">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm border border-zinc-200/50 dark:border-zinc-800/50 rounded-xl text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-zinc-500 focus:outline-none focus:border-blue-500/50 transition-colors text-sm"
            />
            {searchQuery && (
              <button
        ...
        <div className="flex items-center justify-between gap-3 mb-6">
          <p className="text-sm text-zinc-600 dark:text-zinc-400">
            Showing <span className="text-zinc-900 dark:text-white font-medium">{filteredCompanies.length > 0 ? Math.min(indexOfFirstItem + 1, filteredCompanies.length) : 0}-{Math.min(indexOfLastItem, filteredCompanies.length)}</span> of <span className="text-zinc-900 dark:text-white font-medium">{filteredCompanies.length}</span> Companies Found
          </p>
        ...
        {filteredCompanies.length === 0 ? (
          ...
          >
            ...
            <h3 className="text-xl font-semibold text-zinc-900 dark:text-white">No companies found</h3>
            <p className="text-zinc-600 dark:text-zinc-400 mt-2">Try adjusting your search terms</p>
            <button
              onClick={() => setSearchQuery("")}
